...
AI_SUGGESTIONS_URL = "/api/ai/suggestions"

MOVE_PIECE_URL = "/api/move/piece"
THROW_START_DICE_URL = "/api/throw_start_dice"
...
async def test_send_in_game_message(client: AsyncClient, token: str):
    ...
    response = await client.post("/api/game/message", json=message_data, headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 400
...
async def test_move_ai(client: AsyncClient, token: str):
    ...
    response = await client.post("/api/move/ai", json=move_data, headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    updated_game = await get_db().matches.find_one({"player1": "testuser"})
    assert updated_game is not None
    assert updated_game["board_configuration"]["points"][3]["player1"] == 1
...
async def test_request_timeout(client: AsyncClient, token: str):
    await clear_matches()
    await create_started_match("testuser", "testuser2")
    response = await client.post("/api/game/request_timeout", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 400
...
async def test_propose_double(client: AsyncClient, token: str):
    ...
    response = await client.post("/api/game/double/propose", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
...
async def test_accept_double(client: AsyncClient, token: str):
    ...
    response = await client.post("/api/game/double/accept", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
...
async def test_reject_double(client: AsyncClient, token: str):
    ...
    response = await client.post("/api/game/double/reject", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
...
async def test_quit_game(client: AsyncClient, token: str):
    await clear_matches()
    await create_started_match("testuser", "testuser2")
    response = await client.post("/api/game/quit", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
