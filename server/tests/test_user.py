...
async def test_get_top5_and_me(client: AsyncClient, token: str):
    response = await client.get("/api/users/top5_and_me", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    data = response.json()
...
async def test_get_top5_and_me_google(client: AsyncClient, token: str):
    ...
    response = await client.post("/api/users/top5_and_me_google", json=email_list, headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert data[-1]["username"] == "testuser"
...
async def test_get_user_rating(client: AsyncClient, token: str):
    response = await client.get("/api/users/get_user_rating", params={"username": "testuser"}, headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert isinstance(response.json(), int)
    response = await client.get("/api/users/get_user_rating", params={"username": "not_existing_user"}, headers={"Authorization": f"Bearer {token}"})
