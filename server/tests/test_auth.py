...
async def test_password_recovery(client: AsyncClient):
    email = "testuser@example.com"
    response = await client.post("/api/password-recovery", json={"email": email})
    assert response.status_code == 200
...
async def test_password_reset(client: AsyncClient, token: str):
    ...
    response = await client.post("/api/password-reset", json={"token": token, "new_password": new_password})
    assert response.status_code == 200
...
async def test_google_login(mock_verify_oauth2_token, client: AsyncClient):
    ...
    response = await client.post("/api/google-login", json={"accessToken": "mock_code"})
    assert response.status_code == 200
    assert "access_token" in response.json()
