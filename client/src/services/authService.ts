...
export const registerOrLogin = async (
	...
	const response = await axios.post(
		`${import.meta.env.VITE_API_URL}/api/${isRegistering ? 'register' : 'token'}`,
...
export const requestPasswordRecovery = async (email: string) => {
  return await axios.post(`${import.meta.env.VITE_API_URL}/api/password-recovery`, { email })
}

export const resetPassword = async (password: string, token: string) => {
  return await axios.post(`${import.meta.env.VITE_API_URL}/api/password-reset`, { 'new_password': password, token })
}
  ...
  return async response => {
    ...
    if (code)
      try {
        ...
        const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/api/google-login`, { accessToken: accessToken })
