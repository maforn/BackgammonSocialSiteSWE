    ...
    <div class="w-screen h-screen flex flex-col pt-20 items-center">
      ...
      <div v-if="myData"
        ...
        <div class="flex justify-center items-center pl-3 py-2 gap-x-2 self-center" v-if="isGoogle">
          <label for="google-friends-switch" class="text-right">Show Google Friends Leaderboard</label>
          <input type="checkbox" id="google-friends-switch" @change="toggleGoogleFriends" v-model="showGoogleFriends" />
        </div>

        <div v-for="u in usersData.slice(0, usersData.length-1)"
             class="leaderboard-object flex w-5/6 flex-col justify-evenly gap-3">
...
import { getTop5AndMe, getTop5AndMeGoogle } from '@/services/ratingService'
...
export default defineComponent({
  ...
  setup() {
    ...
    const showGoogleFriends = ref(false)
    ...
    return { usersData, myData, userIsInTop, showGoogleFriends, isGoogle }
  },
  methods: {
    async goHome() {
    ...
    async toggleGoogleFriends() {
      if (this.showGoogleFriends) {
        this.usersData = await getTop5AndMeGoogle(await getGoogleContactsEmails())
      } else {
        this.usersData = await getTop5AndMe()
      }
    }
  }
})
