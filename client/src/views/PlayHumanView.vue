      ...
      <div
        ...
        <div
          ...
          <input v-model="searchQuery" @input="onInput" class="flex-grow pl-3 py-2 outline-none bg-gray-900 text-white"
                 :placeholder="inviteGoogleFriends ? 'Search by email...' : 'Search by username...'" />
          ...
          <ul v-if="showDropdown"
            ...
            <li v-for="item in inviteGoogleFriends ? filteredEmails : filteredUsers" :key="item.username || item"
                class="px-4 py-2 hover:bg-gray-200 cursor-pointer text-black"
                @click="inviteGoogleFriends ? selectEmail(item) : selectUser(item)">
              {{ item.username || item }}
            </li>
          </ul>
        </div>
...
import { getGoogleContactsEmails, getRandomOpponentService, sendInviteService } from '@/services/invitesService'
...
export default defineComponent({
  ...
  methods: {
    ...
    async sendInvite() {
      try {
        await sendInviteService(this.searchQuery, this.rounds_to_win, this.inviteGoogleFriends)
        this.searchQuery = ''
        this.hasSelectedOpponent = false
      } catch (error) {
    ...
    async sendRandomInvite() {
      try {
        ...
        await sendInviteService(opponent_username, this.rounds_to_win, false)
      } catch (error) {
    ...
    toggleGoogleFriends() {
      if (this.inviteGoogleFriends) {
        getGoogleContactsEmails().then((data) => {
          this.emails = data
        })
      }
    }
