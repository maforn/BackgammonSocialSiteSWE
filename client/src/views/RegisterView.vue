<template>
	<div class="font-[sans-serif] bg-gray-50 flex items-center md:h-screen p-4">
		<div class="w-full max-w-4xl max-md:max-w-xl mx-auto">
			<div
				class="grid md:grid-cols-2 gap-16 w-full sm:p-8 p-6 shadow-md rounded-md overflow-hidden pl-3 py-2 bg-gray-600 text-white"
			>
				...
				<form class="w-full" method="POST" @submit.prevent="registerUser">
					...
					<p v-if="showRegisterForm" class="text-sm mt-6 text-center">
					...
					<p v-else class="text-sm mt-6 text-center">
				...
				<button
					@click="goBack"
					class="absolute bottom-4 left-4 px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-800"
				>
					Back
				</button>
			</div>
				...
				<div class="max-h-96 overflow-y-auto" id="tos-text">
					...
					<ul>
						...
						<li>
							<p>
								...
								or &quot;Our&quot; in this Agreement) refers to Social Backgammon.
							</p>
						</li>
						<li>
							<p>
						...
						<li>
							<p>
								...
								<a href="PLACEHOLDER_URL" rel="external nofollow noopener" target="_blank">PLACEHOLDER_URL</a>
							</p>
						</li>
						<li>
							<p>
					...
					<ul>
						<li>
							...
							<a href="PLACEHOLDER_URL/contact" rel="external nofollow noopener" target="_blank"
								>PLACEHOLDER_URL/contact</a
							>
						</li>
					</ul>
				</div>
				<button @click="toggleTerms" class="px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-800">
					Close
				</button>
			</div>
		</div>
		...
		const registerUser = async () => {
			try {
				...
				if (axios.isAxiosError(error) && error.response) {
					errorMessage.value = typeof(error.response.data.detail) == 'object' ? error.response.data.detail[0].msg : error.response.data.detail
				} else {
