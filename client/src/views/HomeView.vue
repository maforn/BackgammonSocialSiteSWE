        ...
        <ul v-else class="max-h-[80vh] overflow-auto">
          <li v-for="(invite,index) in invites" :key="index" class="flex flex-col justify-center items-center">
            <button @click="acceptInvite(index)"
              ...
              <span class="bg-indigo-100 ms-2 text-indigo-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-indigo-900 dark:text-indigo-300">First to {{invite.first_to}} </span>
            </button>
          </li>
        </ul>
      </div>
