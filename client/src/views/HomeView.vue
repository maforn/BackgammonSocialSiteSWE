<template>
	<div class="w-screen h-screen flex flex-col pt-20 content-center items-center lg:pl-5">
		...
		<div class="flex flex-col mt-20 w-4/5 sm:w-3/5 lg:w-2/5 gap-4 text-lg content-center">
			<router-link
				:to="hasSuspendedGame ? '/game' : '/human'"
			...
			>
				...
				<div class="w-full h-full flex justify-center items-center pr-8 uppercase font-medium">Play AI</div>
			</router-link>
			<router-link
				to="/"
				class="flex justify-end items-center pl-3 py-2 bg-white text-black rounded-r-full rounded-l-full hover:bg-gray-300 shadow-md"
			>
				...
				<div class="w-full h-full flex justify-center items-center pr-8 uppercase font-medium">Tournament</div>
			</router-link>
			<router-link
				to="/"
				class="flex justify-end items-center pl-3 py-2 bg-white text-black rounded-r-full rounded-l-full hover:bg-gray-300 shadow-md"
			>
				...
				<div class="w-full h-full flex justify-center items-center pr-8 uppercase font-medium">Leaderboards</div>
			</router-link>
			<button
				id="show-invites-btn"
				to="/"
				@click.prevent="showInvites"
				class="flex justify-end items-center pl-3 py-2 bg-white text-black rounded-r-full rounded-l-full hover:bg-gray-300 shadow-md"
			>
				<div class="circle flex items-center justify-center rounded-full">
					<v-icon name="io-mail-sharp" class="text-white" scale="1.5" />
				</div>
				<div class="w-full h-full flex justify-center items-center pr-8 uppercase font-medium">Receive invites</div>
			</button>
		</div>
			...
			<div class="overlay-content relative">
				...
				<ul v-else class="max-h-[80vh] overflow-auto">
					<li v-for="(invite, index) in invites" :key="index" class="flex flex-col justify-center items-center">
						...
						>
							Invite from&nbsp;
							<p class="font-black">{{ invite.player1 }}</p>
							<span
								class="bg-indigo-100 ms-2 text-indigo-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-indigo-900 dark:text-indigo-300"
								>First to {{ invite.first_to }}
							</span>
						</button>
					</li>
				</ul>
