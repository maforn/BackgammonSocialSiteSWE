...
import {
  ...
  doRandomMove,
  updateAISuggestions,
  getPiecesSummary, isBackgammon, isGammon, NUMBER_OF_PLAYER_PIECES
...
describe('updateAISuggestions', () => {
  ...
  it('should call the AI suggestions endpoint', async () => {
    mock.onPost('/ai/suggestions').reply(200)

    await updateAISuggestions()

    expect(mock.history.post.length).toBe(1)
    expect(mock.history.post[0].url).toBe('/ai/suggestions')
  })

  it('should handle errors gracefully', async () => {
    mock.onPost('/ai/suggestions').reply(500)
